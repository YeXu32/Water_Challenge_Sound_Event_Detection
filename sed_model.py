...
def load_data(_feat_folder, _mono):
    feat_file_fold = os.path.join(_feat_folder, 'mbe_{}.npz'.format('mon' if _mono else 'bin'))
    dmp = np.load(feat_file_fold)
    _X_train, _Y_train,  _X_test, _Y_test = dmp['arr_0'],  dmp['arr_1'],  dmp['arr_2'],  dmp['arr_3'], 
    return _X_train, _Y_train, X_test, Y_test
...
#X, Y = load_data(feat_folder, is_mono)
#X, Y = preprocess_data(X, Y, seq_len, nb_ch)
...
#for i in range(1):
    ...
    if er_overall_1sec_list[i] < best_er:
        ...
        model.save(os.path.join(__models_dir, '{}_model.h5'.format(__fig_name)))
        best_epoch = i
        pat_cnt = 0
